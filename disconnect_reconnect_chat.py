...
class Chat:
    ...
    def return_disconnect(self):
        req = Lcu_core.riot_request("GET", "/chat/v1/session", "")
        req_data = req.json()
    ...
    def disconnect(self):
        body = {"config": "disable"}
        response = Lcu_core.riot_request("POST", "/chat/v1/suspend", body)
        print(response.text)  # Troquei `input` por `print` para exibir a resposta

    def reconnect(self):
        response = Lcu_core.riot_request("POST", "/chat/v1/resume", "")
